...
def get_starting_set(number_of_ambs, radius, display=False):
    global top_n, r1
    if type(number_of_ambs) is not int:
        raise Exception ("get_starting_set expects an int")
    top_n = number_of_ambs
    r1 = radius
    return find_starting_set(display=display)
